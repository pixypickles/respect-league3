...
const secretInfoBtnEl = document.getElementById("secretInfoBtn");
const secretInfoScreenEl = document.getElementById("secretInfoScreen");
const secretInfoBackBtnEl = document.getElementById("secretInfoBackBtn");
const oniBattleBtnEl = document.getElementById("oniBattleBtn");
const momotaroBattleBtnEl = document.getElementById("momotaroBattleBtn");
...
const oniTagBtnEl = document.getElementById("oniTagBtn");
...
function ensureUnlockedBossTeams(){
  if(!bossTeamsUnlocked()) return;
  const base=TEAM_DEFS.find(t=>t.id==="fst") || TEAM_DEFS[0];
  if(!TEAM_DEFS.some(t=>t.id==="oni_team")){
    TEAM_DEFS.push({...base,id:"oni_team",name:"鬼",color:"#f2c21b",secondary:"#111111",bossTeam:"oni"});
  }
  if(!TEAM_DEFS.some(t=>t.id==="momotaro_team")){
    TEAM_DEFS.push({...base,id:"momotaro_team",name:"桃太郎",color:"#ffffff",secondary:"#d22",bossTeam:"momotaro"});
  }
}
...
function hasMomotaroSuperArmor(p){
  return !!(p && p.momotaroRole==="momotaro");
}

function hasDevelopedSuperPhysical(p){
  if(!p) return false;

  // Momotaro always has boss-grade super armor.
  if(hasMomotaroSuperArmor(p)) return true;
...
function buildDevelopmentState(){
  // v164: selected team + 3 random opponents = 4-team single round robin.
...
function displayOpponentName(){
  ...
  const kind=bossKindForTeamId(opponentTeamId);
  if(kind==="oni") return "鬼";
  if(kind==="momotaro") return "桃太郎";
  return teamDef(opponentTeamId).name;
}
function displaySelectedName(){
  const kind=bossKindForTeamId(selectedTeamId);
  if(kind==="oni") return "鬼";
  if(kind==="momotaro") return "桃太郎";
...
function setMenuScreen(which){
  // v164: prevent the same touch from falling through into the newly shown screen.
...
function oniVictoryKey(){ return `oniVictory:${selectedTeamId}`; }
function momotaroVictoryKey(){ return `momotaroVictory:${selectedTeamId}`; }
function hasBeatenOni(){ return localStorage.getItem(oniVictoryKey())==="1"; }
function hasBeatenMomotaro(){ return localStorage.getItem(momotaroVictoryKey())==="1"; }

function momotaroBattleUnlocked(){
  const s=developmentStatsFor(selectedTeamId);
  return (s.speed||0)>=7 && (s.kick||0)>=7 && (s.physical||0)>=7 && hasBeatenOni();
}
function refreshMomotaroBattleButton(){
  if(!momotaroBattleBtnEl) return;
  momotaroBattleBtnEl.style.display=momotaroBattleUnlocked()?"":"none";
}
function bossTeamsUnlocked(){
  return hasBeatenMomotaro();
}
...
function applyDedicatedOniRoster(){
  // Opponent is always red in the hidden Oni battle.
  for(const p of teams.red){
    clearBossVisualFlags(p);
    p.oni=true;
    p._dedicatedBossRole="oni";
    p._bossTraitsApplied=true;
    p.speed=Math.max(p.speed||0,270);
    p.devKickMult=Math.max(p.devKickMult||1,1.85);
    p.devPhysical=Math.max(p.devPhysical||0,10);
  }
}

function applyDedicatedMomotaroRoster(){
  // Opponent is always red in the hidden Momotaro battle.
  const roster=teams.red;
  for(const p of roster){
    clearBossVisualFlags(p);
    p._bossTraitsApplied=false;
  }

  const field=roster.filter(p=>p.role!=="gk");
  const roles=["momotaro","dog","monkey","pheasant"];

  field.forEach((p,idx)=>{
    p.momotaro=true;
    p._dedicatedBossRole="momotaro";
    ...
    // Momotaro 10-ish, companions speed 15-ish.
    p.speed=Math.max(p.speed||0,idx===0?350:500);
    p.devKickMult=Math.max(p.devKickMult||1,idx===0?2.25:1.35);
    p.devPhysical=Math.max(p.devPhysical||0,idx===0?10:9);
  ...
  if(gk){
    gk.momotaro=false;
    gk._dedicatedBossRole="momotaro";
    gk.momotaroRole=null;
    gk.momotaroGK=true;
    gk.oni=true;
    gk._bossTraitsApplied=true;
    gk.speed=Math.max(gk.speed||0,210);
    gk.devPhysical=Math.max(gk.devPhysical||0,10);
  }
}
...
function startMomotaroBattle(){
  ...
  momotaroState.peachDefenseLock=0;

  // Clear previous match identities before the common match builder runs.
  for(const p of [...teams.blue,...teams.red]){
    clearBossVisualFlags(p);
    p._bossTraitsApplied=false;
  }

  // The backing opponent ID is only used to build a normal roster.
  // The dedicated Momotaro roster is applied AFTER startMatch finishes.
  const opponent=(TEAM_DEFS.find(t=>t.id==="fst")?.id)||TEAM_DEFS[TEAM_DEFS.length-1].id;
  startMatch(opponent);

  // IMPORTANT: this must be the final identity assignment for this match.
  applyDedicatedMomotaroRoster();
}

function momotaroPlayer(){
  return teams.red.find(p=>p.momotaroRole==="momotaro")||null;
}


function playableMomotaroMode(){
  return gameMode==="free" || gameMode==="daycup" ||
         gameMode==="deathmatch" || gameMode==="deathmatch-explosive" ||
         gameMode==="deathmatch-dragon";
}

function fireCpuPeachPostShot(p){
  if(!p || p.controlled || ball.owner!==p || p.momotaroRole!=="momotaro") return false;
  p.receiveLock=0;
  p.aiTimer=0;
  p.scanTimer=0;
  ball.passTarget=null;
  ...
  const goalX=p.team==="blue" ? COURT.x+COURT.w+18 : COURT.x-18;
  const postY=H/2+(Math.random()<.5?-1:1)*(GOAL_H/2-5);
  const n=norm(goalX-p.x,postY-p.y);
  p.dirX=n.x;p.dirY=n.y;

  convertingPeachShot=true;
  kickBall(p,n.x,n.y,1360,0,true,null);
...
function firePeachPostShot(p){
  if(p && p.controlled && !(input.shootDown || input.actionPriorityTimer>0)) return false;

  if(!p || ball.owner!==p) return false;
  const bossMatchMomotaro=gameMode==="momotaro" && p.team==="red" && p.momotaroRole==="momotaro";
  const playableMomotaro=p.momotaro && p.momotaroRole==="momotaro";
  if(!(bossMatchMomotaro || playableMomotaro)) return false;

  // Aim at the OPPONENT goal based on Momotaro's team.
  // Blue attacks right, red attacks left.
  const attackRight=p.team==="blue";
  const goalX=attackRight ? COURT.x+COURT.w+18 : COURT.x-18;

  // Choose upper/lower post of the opponent goal.
  const postY=H/2+(Math.random()<.5?-1:1)*(GOAL_H/2-5);
  const n=norm(goalX-p.x,postY-p.y);

  p.dirX=n.x;p.dirY=n.y;
  convertingPeachShot=true;
  kickBall(p,n.x,n.y,1320,0,true,null);
  convertingPeachShot=false;
...
function updateMomotaroBattle(dt){
  if(gameMode!=="momotaro" && !momotaroState.peachShot) return;
  ...
  if(momotaroState.peachShot && !ball.owner){
    ...
    // Before impact, continually home toward the selected post.
    if(!momotaroState.peachHitPost){
      const shooter=momotaroState.peachShooter;
      const attackRight=shooter && shooter.team==="blue";
      const tx=attackRight ? COURT.x+COURT.w-2 : COURT.x+2;
      const ty=momotaroState.peachTargetPostY;
      const n=norm(tx-ball.x,ty-ball.y);
      const speed=Math.max(1080,Math.hypot(ball.vx,ball.vy));
      ball.vx=lerp(ball.vx,n.x*speed,dt*7.5);
      ball.vy=lerp(ball.vy,n.y*speed,dt*7.5);

      if(Math.hypot(ball.x-tx,ball.y-ty)<24){
        momotaroState.peachHitPost=true;
        // Rebound explicitly toward the inside of the goal.
        const shooter=momotaroState.peachShooter;
        const attackRight=shooter && shooter.team==="blue";
        const insideX=attackRight ? COURT.x+COURT.w+38 : COURT.x-38;
        const inside=norm(insideX-ball.x,(H/2)-ball.y);
        ball.vx=inside.x*1040;
        ball.vy=inside.y*1040;
        ball.vz=Math.min(ball.vz,22);
        showMessage("POST IN!",.28);
      }
    }else{
      // After post impact, keep it pulling into the net until goal detection.
      const shooter=momotaroState.peachShooter;
      const attackRight=shooter && shooter.team==="blue";
      const insideX=attackRight ? COURT.x+COURT.w+42 : COURT.x-42;
      const inside=norm(insideX-ball.x,(H/2)-ball.y);
      const speed=Math.max(900,Math.hypot(ball.vx,ball.vy));
      ball.vx=lerp(ball.vx,inside.x*speed,dt*12);
      ball.vy=lerp(ball.vy,inside.y*speed,dt*12);
    }
  }
}
...
function startOniBattle(){
  ...
  // Final assignment after all common match/reset code.
  applyDedicatedOniRoster();
}
...
function oniShootAtPlayerGoal(p,type="normal"){
  ...
  const playableOni=p.oni && !p.momotaroGK && bossKindForTeamId(sideTeamIdForPlayer(p))==="oni";
...
function setupDeathmatchLines(){
  // v164: vertical hazard lines only, evenly spaced.
...
const oniTagState={
  ...
  touchLock:0,
  blast:null,
  noRepickupPlayer:null,
  noRepickupTimer:0
};

function oniTagParticipants(){
  return [...teams.blue,...teams.red].filter(p=>p.oniTagParticipant);
}

function startOniTag(){
  gameMode="oni-tag";
  selectedMatchSeconds=30;
  oniTagState.active=true; oniTagState.oni=null; oniTagState.lastTouch=null;
  oniTagState.touchLock=0; oniTagState.blast=null;
  oniTagState.noRepickupPlayer=null; oniTagState.noRepickupTimer=0;

  const ordinary=TEAM_DEFS.filter(t=>t.id!=="oni_team" && t.id!=="momotaro_team");
  const opp=(ordinary.find(t=>t.id!==selectedTeamId)?.id)||ordinary[0].id;
  startMatch(opp);

  const blue=teams.blue.filter(p=>p.role!=="gk").slice(0,3);
  const red=teams.red.filter(p=>p.role!=="gk").slice(0,3);
  const ps=[...blue,...red];

  for(const p of [...teams.blue,...teams.red]){
    p.oniTagParticipant=ps.includes(p);
    if(!p.oniTagParticipant){
      p.x=-5000;p.y=-5000;p.vx=p.vy=0;
    }else{
      clearBossVisualFlags(p);p._bossTraitsApplied=false;
      p.stun=0;p.stagger=0;
    }
  }

  const spots=[
    [COURT.x+COURT.w*.24,COURT.y+COURT.h*.25],[COURT.x+COURT.w*.24,COURT.y+COURT.h*.75],
    [COURT.x+COURT.w*.39,COURT.y+COURT.h*.50],[COURT.x+COURT.w*.61,COURT.y+COURT.h*.25],
    [COURT.x+COURT.w*.61,COURT.y+COURT.h*.75],[COURT.x+COURT.w*.76,COURT.y+COURT.h*.50]
  ];
  ps.forEach((p,i)=>{p.x=spots[i][0];p.y=spots[i][1];p.vx=p.vy=0;});

  ball.owner=null;ball.x=W/2;ball.y=H/2;ball.z=0;
  ball.vx=ball.vy=ball.vz=0;ball.lastTouch=null;ball.passTarget=null;ball.passFrom=null;
  const controlledTagPlayer=ps.find(p=>p.controlled) || ps[0];
  oniTagState.oni=controlledTagPlayer;
  matchLeft=30;
  scoreEl.textContent="鬼ごっこ";
  showMessage("6人全員が敵！ 最後にボールへ触れた人が鬼！",1.8);
}

function triggerOniTagBlast(p){
  oniTagState.blast={x:ball.x,y:ball.y,t:.32,r:8};
  for(const q of oniTagParticipants()){
    if(q===p) continue;
    const dx=q.x-ball.x,dy=q.y-ball.y,d=Math.hypot(dx,dy)||1;
    if(d<105){
      const f=(1-d/105)*280;
      q.vx+=(dx/d)*f;q.vy+=(dy/d)*f;q.stagger=Math.max(q.stagger||0,.16);
    }
  }
}


function moveTagCPU(p,tx,ty,speedMul=1){
  const dx=tx-p.x,dy=ty-p.y;
  const n=norm(dx,dy);
  const base=(p.speed||180)*speedMul;
  p.vx=lerp(p.vx,n.x*base,.18);
  p.vy=lerp(p.vy,n.y*base,.18);
  if(Math.abs(n.x)>.05) p.dirX=n.x;
  if(Math.abs(n.y)>.05) p.dirY=n.y;
}


function nearestOniTagTarget(from){
...
function fireOniTagAutoShot(p,baseSpeed=520){
  if(gameMode!=="oni-tag" || !p || ball.owner!==p) return false;

  const target=nearestOniTagTarget(p);
  ...
  // In Oni Tag, use a normal tag projectile regardless of development pierce.
  // Contact must always register instead of ghosting through a target.
  ball.developedPierce=false;
  ball.developedPierceShooter=null;

  kickBall(p,n.x,n.y,baseSpeed,0,false,target);

  oniTagState.noRepickupPlayer=p;
  oniTagState.noRepickupTimer=.38;
  return true;
}

function updateOniTagCPU(p,dt){
  if(!p || p.controlled || !p.oniTagParticipant) return;

  const oni=oniTagState.oni;
  const participants=oniTagParticipants();

  if(p===oni){
    // The Oni's job is to collect the ball and force somebody else to touch it.
    if(ball.owner===p){
      let target=null,best=1e9;
      ...
      if(target){
        // Approach just enough to line up, then fire the ball at the nearest player.
        const d=Math.hypot(target.x-p.x,target.y-p.y);
        if(d<380 && p.cooldown<=0){
          if(fireOniTagAutoShot(p,540)){
            p.cooldown=.28;
            return;
          }
        }
        moveTagCPU(p,target.x,target.y,1.08);
      }
      return;
    }

    // Loose/other-owned ball: chase the ball, not a football opponent.
    moveTagCPU(p,ball.x,ball.y,1.15);
    return;
  }

  // Everyone except the Oni is an individual opponent.
  // They avoid both the ball and the Oni instead of marking the ball carrier.
  let ax=0,ay=0;

  const db=Math.hypot(p.x-ball.x,p.y-ball.y)||1;
  if(db<260){
    ax+=(p.x-ball.x)/db*(280-db);
    ay+=(p.y-ball.y)/db*(280-db);
  }

  if(oni){
    const do_=Math.hypot(p.x-oni.x,p.y-oni.y)||1;
    if(do_<300){
      ax+=(p.x-oni.x)/do_*(320-do_)*1.2;
      ay+=(p.y-oni.y)/do_*(320-do_)*1.2;
    }
  }

  // Also spread away from other runners so everyone does not bunch up.
  for(const q of participants){
    if(q===p) continue;
  ...
  // If there is no immediate threat, drift toward a random-ish safe point
  // based on the player's stable roster index.
  if(Math.hypot(ax,ay)<8){
    const idx=participants.indexOf(p);
    const safeX=COURT.x+COURT.w*(.18+((idx*37)%64)/100);
    const safeY=COURT.y+COURT.h*(.18+((idx*53)%64)/100);
    ax=safeX-p.x;
    ay=safeY-p.y;
  }

  const n=norm(ax,ay);
  const base=(p.speed||180)*1.02;
  p.vx=lerp(p.vx,n.x*base,.16);
  p.vy=lerp(p.vy,n.y*base,.16);
  if(Math.abs(n.x)>.05) p.dirX=n.x;
  if(Math.abs(n.y)>.05) p.dirY=n.y;

  // Non-Oni CPUs never intentionally trap/collect the ball.
  if(ball.owner===p){
    // Immediately dump it away so they do not start playing football.
    const away=oni ? norm(p.x-oni.x,p.y-oni.y) : norm(p.x-W/2,p.y-H/2);
    kickBall(p,away.x,away.y,360,0,false,null);
    p.cooldown=.18;
  }
}

function updateOniTag(dt){

  oniTagState.noRepickupTimer=Math.max(0,oniTagState.noRepickupTimer-dt);
  if(oniTagState.noRepickupTimer<=0) oniTagState.noRepickupPlayer=null;


  // Room walls: bounce instead of goals/out-of-bounds.
  const left=COURT.x+BALL_R+10;
  const right=COURT.x+COURT.w-BALL_R-10;
  const top=COURT.y+BALL_R+10;
  const bottom=COURT.y+COURT.h-BALL_R-10;
  if(ball.x<left){ball.x=left;ball.vx=Math.abs(ball.vx)*.82;}
  if(ball.x>right){ball.x=right;ball.vx=-Math.abs(ball.vx)*.82;}
  if(ball.y<top){ball.y=top;ball.vy=Math.abs(ball.vy)*.82;}
  if(ball.y>bottom){ball.y=bottom;ball.vy=-Math.abs(ball.vy)*.82;}

  // Easier loose-ball pickup, but the shooter cannot instantly re-collect its own shot.
  if(!ball.owner && ball.z<30 && oniTagState.oni){
    const p=oniTagState.oni;
    const blocked=(oniTagState.noRepickupTimer>0 && oniTagState.noRepickupPlayer===p);
    const d=Math.hypot(p.x-ball.x,p.y-ball.y);
    const speed=Math.hypot(ball.vx,ball.vy);
    const pickupRadius=speed<170?58:48;
    if(!blocked && d<pickupRadius){
      ball.owner=p;
      ball.lastTouch=p;
  ...
  // Oni Tag only: forgiving player/ball hitbox.
  // A fast loose ball passing slightly beside a runner still counts as a touch.
  if(!ball.owner && ball.z<34 && oniTagState.touchLock<=0){
    const speed=Math.hypot(ball.vx,ball.vy);
    if(speed>70){
      let hit=null,hitD=1e9;
      for(const p of oniTagParticipants()){
        if(p===oniTagState.lastTouch) continue;
        const d=Math.hypot(p.x-ball.x,p.y-ball.y);
        if(d<hitD){hit=p;hitD=d;}
      }
      const tagHitRadius=PLAYER_R+BALL_R+26;
      if(hit && hitD<tagHitRadius){
        // Tag contact always wins over normal football/pierce behavior.
        ball.developedPierce=false;
        ball.developedPierceShooter=null;
        ball.lastTouch=hit;

        const n=norm(ball.x-hit.x,ball.y-hit.y);
        const nx=(Math.abs(n.x)+Math.abs(n.y)>.001)?n.x:1;
        const ny=(Math.abs(n.x)+Math.abs(n.y)>.001)?n.y:0;
        ball.x=hit.x+nx*(tagHitRadius+2);
        ball.y=hit.y+ny*(tagHitRadius+2);
        ball.vx=nx*Math.max(150,speed*.55);
        ball.vy=ny*Math.max(150,speed*.55);
      }
    }
  }

  oniTagState.touchLock=Math.max(0,oniTagState.touchLock-dt);
  if(oniTagState.blast){
    oniTagState.blast.t-=dt;oniTagState.blast.r+=dt*300;
    if(oniTagState.blast.t<=0) oniTagState.blast=null;
  }
  const t=ball.lastTouch;
  if(t&&t.oniTagParticipant&&t!==oniTagState.lastTouch&&oniTagState.touchLock<=0){
    oniTagState.lastTouch=t;oniTagState.oni=t;oniTagState.touchLock=.16;
    triggerOniTagBlast(t);showMessage("鬼交代！",.45);
  }
}

function drawOniTagEffects(){
  if(gameMode!=="oni-tag") return;
  ctx.save();
  ctx.fillStyle="#d71920";ctx.beginPath();ctx.arc(ball.x,ball.y,BALL_R+3,0,Math.PI*2);ctx.fill();
  ctx.strokeStyle="#ffd36a";ctx.lineWidth=2;ctx.stroke();
  if(oniTagState.blast){
    ctx.globalAlpha=Math.max(0,oniTagState.blast.t/.32);
    ctx.strokeStyle="#ff5a36";ctx.lineWidth=7;ctx.beginPath();
    ctx.arc(oniTagState.blast.x,oniTagState.blast.y,oniTagState.blast.r,0,Math.PI*2);ctx.stroke();
  }
  ctx.restore();
  if(oniTagState.oni){
    const p=oniTagState.oni;
    ctx.save();ctx.strokeStyle="#ff2f2f";ctx.lineWidth=6;ctx.beginPath();
    ctx.arc(p.x,p.y,PLAYER_R+10,0,Math.PI*2);ctx.stroke();
    ctx.fillStyle="#ff2f2f";ctx.font="bold 18px sans-serif";ctx.textAlign="center";
    ctx.fillText("鬼",p.x,p.y-PLAYER_R-15);ctx.restore();
  }
}
...
function explosiveGKImpact(p){
  if(gameMode!=="deathmatch-explosive" || !p || p.role!=="gk") return false;

  // Never allow an explosive ball to stay attached to a keeper.
  if(ball.owner===p){
    ball.owner=null;
  }

  if(ball.explosiveGKLock>0) return false;
  if(dist(p,ball)>54 || ball.z>46) return false;

  ball.explosiveGKLock=.35;

  // One impact = one explosion.
  explodeDangerBall("gk");

  const n=norm(ball.x-p.x,ball.y-p.y);
  const fallback=norm(p.team==="blue"?1:-1,0);
  const dir=(Math.abs(n.x)+Math.abs(n.y)>.01)?n:fallback;

  p.gkFall=Math.max(p.gkFall||0,.95);
  p.stagger=Math.max(p.stagger||0,.75);
  p.vx=-dir.x*320;
  p.vy=-dir.y*220;

  // The ball becomes loose immediately after the explosion.
  ball.owner=null;
  ball.passTarget=null;
  ball.passFrom=null;
  ball.shot=false;
  ball.vx=dir.x*rand(220,340);
  ball.vy=dir.y*rand(160,300);
  ball.vz=Math.max(ball.vz,85);
  ball.touchGrace=.16;
  ball.lastTouch=p;
...
function updateExplosiveBall(dt){
  // Safety: explosive ball must never remain attached to a keeper.
  if(ball.owner && ball.owner.role==="gk"){
    const gk=ball.owner;
    ball.owner=null;
    if(ball.explosiveGKLock<=0){
      explosiveGKImpact(gk);
    }
  }

  // While another player owns it, do not run wall/impact explosion loops.
...
function bazookaAimDirection(p){
  // v164: forward is always the attacking direction, never toward own goal.
...
function startMatch(opponentId){
  ...
  // v164: roster has finished initializing; now force boss graphics/stats.

  // v164: all players now exist; rebuild boss identities from CURRENT selections.
...
let momotaroRewardPointsLeft=0;

function renderMomotaroRewardChoices(){
  const stats=developmentStatsFor(selectedTeamId);
  developmentInfoEl.textContent=
    `残り育成ポイント ${momotaroRewardPointsLeft}  /  走力+${stats.speed}  キック力+${stats.kick}  フィジカル+${stats.physical}`;
  resultActionsEl.innerHTML="";
  if(momotaroRewardPointsLeft<=0){
    tournamentProgressEl.textContent="桃太郎撃破ボーナス +3 獲得 / 鬼・桃太郎チーム解禁";
    addResultButton("桃太郎と再戦",true,startMomotaroBattle);
    addResultButton("チーム選択へ",false,returnToMainMenu);
    return;
  }
  tournamentProgressEl.textContent=`桃太郎撃破ボーナス：あと${momotaroRewardPointsLeft}ポイント選択`;
  addResultButton("走力 +1",true,()=>applyMomotaroRewardPoint("speed"));
  addResultButton("キック力 +1",true,()=>applyMomotaroRewardPoint("kick"));
  addResultButton("フィジカル +1",true,()=>applyMomotaroRewardPoint("physical"));
}
function applyMomotaroRewardPoint(kind){
  if(momotaroRewardPointsLeft<=0) return;
  const stats=developmentStatsFor(selectedTeamId);
  stats[kind]=(stats[kind]||0)+1;
  saveDevelopmentStats(selectedTeamId,stats);
  momotaroRewardPointsLeft--;
  renderMomotaroRewardChoices();
}
...
function finishMatch(){
  ...
  if(gameMode==="oni-tag"){
    resultKickerEl.textContent="ONI TAG";
    resultTitleEl.textContent="TIME UP";
    resultScoreEl.textContent=oniTagState.oni?"最後に鬼だった1人の負け":"終了";
    tournamentProgressEl.textContent="6人全員の個人戦 / チーム勝敗なし";
    oniTagState.active=false;
    addResultButton("もう一度",true,startOniTag);
    addResultButton("モード選択へ",false,()=>{gamePhase="menu";setMenuScreen(modeScreenEl);});
  ...
  refreshMomotaroBattleButton();
  ...
  }else if(gameMode==="momotaro"){
    if(scoreBlue>scoreRed){
      localStorage.setItem(momotaroVictoryKey(),"1");
      resultTitleEl.textContent="桃太郎撃破！";
      momotaroRewardPointsLeft=3;
      renderMomotaroRewardChoices();
    }else{
      resultTitleEl.textContent=scoreBlue<scoreRed?"桃太郎に敗北":"DRAW";
      tournamentProgressEl.textContent="勝利すると育成ポイント+3";
      addResultButton("桃太郎と再戦",true,startMomotaroBattle);
      addResultButton("チーム選択へ",false,returnToMainMenu);
    }
    ...
    if(scoreBlue>scoreRed){
      localStorage.setItem(oniVictoryKey(),"1");
      refreshMomotaroBattleButton();
...
function resetKickoff(team="blue") {
  ...
  ball.z=0; ball.vx=ball.vy=ball.vz=0; ball.shot=false;
    ...
    ball.oniBlastShot=false;
    ball.oniBlastConsumed=false;
    ball.explosiveGKLock=0; ball.passTarget=null;
  ...
  // v164: preserve current boss-team identities after kickoff reset.

  // v164: identity assignment must happen last.
  if(gameMode==="momotaro"){
    applyDedicatedMomotaroRoster();
  }else if(gameMode==="oni"){
    applyDedicatedOniRoster();
  ...
  // v164: kickoff possession is assigned LAST.
  ...
  // v164: Oni Tag room keeps nonparticipants hidden.
  if(gameMode==="oni-tag"){
    for(const p of [...teams.blue,...teams.red]){
      if(!p.oniTagParticipant){p.x=-5000;p.y=-5000;p.vx=p.vy=0;}
    }
  }
}
...
function kickBall(p, dx,dy, speed, lift=0, shot=false, target=null) {
  ...
  // v164: Oni Tag SHOT always auto-aims at the nearest other player.
  // This also normalizes developed/piercing shots so they still count as tag contact.
  if(
    gameMode==="oni-tag" &&
    p &&
    p.controlled &&
  ...
  if(
    ...
    p &&
    p.controlled &&
    (input.shootDown || input.actionPriorityTimer>0) &&
    p.oni &&
    !p.momotaroGK &&
    bossKindForTeamId(sideTeamIdForPlayer(p))==="oni" &&
    ball.owner===p
  ){
  ...
  // v164: Peach homing belongs only to Momotaro's own special shot.
  ...
  // v164: shooting during time stop releases the ball immediately.
  if(gameMode==="oni-tag" && timeStopTimer>0 && p && p.controlled && shot){
    timeStopBallFrozen=false;
    timeStopBallSnapshot=null;
  }
  ball.lastTouch=p;
  ...
  // v164: Oni Tag always uses tag-contact projectiles, never goal-piercing shots.
  if(gameMode==="oni-tag"){
    ball.developedPierce=false;
    ball.developedPierceShooter=null;
  }
...
function attemptTrap(p, dt) {
  if(isOniHardPierce()) return false;

  if(ball.developedPierce && !ball.owner) return false;

  // v164: TRAP must not vacuum a loose ball from a distance.
  ...
  if(p.controlled) {
    ...
    if(input.trap || input.trapPressBuffer>0 || input.trapGraceTimer>0 ||
      ...
      // v164: never stop/snap a ball unless it is genuinely at the feet.
  ...
  if(!p.controlled) {
    ...
    if(success) {
      // v164: CPU also needs real contact before claiming/stopping the ball.
...
function aiWithBall(p,dt) {
  ...
  // v164 safety net: Momotaro never uses normal pass/dribble possession AI.
  if(p.momotaro && p.momotaroRole==="momotaro" && !p.controlled){
    if(p.cooldown<=0) fireCpuPeachPostShot(p);
    return;
  }

  // v164: any AI field player may shoot when the chance is clearly good.
...
function updateDedicatedMomotaroCPU(p,dt){
  if(gameMode!=="momotaro" || p.team!=="red" || p.role==="gk" || p.controlled) return false;
  if(!p.momotaroRole) return false;

  if(ball.owner===p){
    if(p.momotaroRole==="momotaro"){
      // Dedicated boss rule: possession = peach shot.
      p.receiveLock=0;
      p.aiTimer=0;
      p.scanTimer=0;
      p.cooldown=0;

      if(fireCpuPeachPostShot(p)){
        p.cooldown=.16;
        return true;
      }

      // Do not fall through into pass AI.
      p.vx*=.82;
      p.vy*=.82;
      return true;
    }

    // Dog / Monkey / Pheasant immediately feed Momotaro.
    const momo=teams.red.find(q=>q.momotaroRole==="momotaro");
    if(momo && momo!==p && p.cooldown<=0){
      doPass(p,momo);
      p.cooldown=.06;
      return true;
    }
...
function updateAI(p,dt) {
  if(updateDedicatedMomotaroCPU(p,dt)) return;


  // v164: CPU Momotaro has absolute priority over every normal football AI branch.
  // This must run before loose-ball logic, safe-pass logic, pressure logic, etc.
  if(
    p &&
    !p.controlled &&
    p.momotaro &&
    ...
    // Ignore long receive/scan/pass delays inherited from normal teams.
    p.receiveLock=0;
    p.aiTimer=0;
    p.scanTimer=0;
    ...
    // Even while cooling down, do NOT fall through into normal pass AI.
    p.vx*=.82;
    p.vy*=.82;
    return;
  }

  // v164: controlled player must never receive CPU boss decisions.
  ...
  if(
    ...
    if(p.momotaroRole==="momotaro"){
      // v164: every CPU Momotaro possession becomes a peach shot.
      if(p.cooldown<=0 && fireCpuPeachPostShot(p)){
        p.cooldown=.32;
        return;
      }
    }else if(p.momotaroRole){
      const momo=momotaroPlayer();
      if(momo && momo!==p && p.cooldown<=0){
        // Dog/Monkey/Pheasant immediately return possession to Momotaro.
        doPass(p,momo);
        p.cooldown=.08;
        return;
      }
  ...
  if(
    (
      ...
      (p.oni && !p.momotaroGK && bossKindForTeamId(sideTeamIdForPlayer(p))==="oni")
    ) &&
    !p.controlled &&
    ball.owner===p &&
    p.role!=="gk"
...
function isOniHardPierce(){
  return gameMode==="oni" &&
         ball.developedPierce &&
         !ball.owner &&
         ball.shot &&
         ball.developedPierceShooter &&
         ball.developedPierceShooter.oni;
}

function blowThroughKeeper(p){
  const n=norm(ball.vx,ball.vy);
  p.gkFall=Math.max(p.gkFall||0,1.35);
  p.stagger=Math.max(p.stagger||0,1.1);
  p.vx=n.x*560;
  p.vy=n.y*260;
  ball.owner=null;
  ball.shot=true;
  ball.developedPierce=true;
  ball.vx=n.x*Math.max(980,Math.hypot(ball.vx,ball.vy)*.995);
  ball.vy=n.y*Math.max(980,Math.hypot(ball.vx,ball.vy)*.995);
}

function updateGK(p,dt) {
  // v164: 鬼蹴・剛 is absolute. No keeper save path may touch it.
  if(isOniHardPierce()){
  ...
  // v164: 鬼蹴・剛 and any developed pierce shot ignore ALL keeper save logic.
  ...
  if(gameMode==="deathmatch-explosive"){
    if(explosiveGKImpact(p)) return;
    // Explosive balls are never held/caught by the keeper.
  ...
  if(slowBall){
    ...
    ball.vx=ball.vy=ball.vz=0;
    ball.z=0;
    if(!isOniHardPierce()) ball.shot=false;
    ball.developedPierce=false;
    ball.developedPierceShooter=null;
    ball.oniBlastShot=false;
    ball.oniBlastConsumed=false;
...
function bossTraitsAllowedForCurrentMatch(){
  const blueKind=bossKindForTeamId(selectedTeamId);
  const redKind=bossKindForTeamId(opponentTeamId);
  if(blueKind==="oni" || blueKind==="momotaro" ||
     redKind==="oni" || redKind==="momotaro") return true;
  return bossTeamsUnlocked();
}

function bossKindForTeamId(teamId){
  const def=TEAM_DEFS.find(t=>t.id===teamId);
  if(!def) return null;

  if(def.bossTeam==="oni" || def.id==="oni_team" || def.name==="鬼") return "oni";
  if(def.bossTeam==="momotaro" || def.id==="momotaro_team" || def.name==="桃太郎") return "momotaro";
  return null;
}

function clearBossVisualFlags(p){
  p.momotaroRole=null;
  p.momotaro=false;
  p.momotaroGK=false;
  p.oni=false;
  p._dedicatedBossRole=null;
}
...
function applyBossTraitsToSide(side,teamId){
  const kind=bossKindForTeamId(teamId);
  if(!kind) return;
  if(!bossTraitsAllowedForCurrentMatch()) return;
  ...
  if(kind==="oni"){
    ...
    if(gk){
      gk.oni=true;
      gk.speed=Math.max(gk.speed||0,205);
      gk.devPhysical=Math.max(gk.devPhysical||0,10);
      gk._bossTraitsApplied=true;
    }
    return;
  ...
  orderedField.forEach((p,idx)=>{
    ...
    p.devKickMult=Math.max(p.devKickMult||1, idx===0 ? 2.25 : 1.35);
    p.devPhysical=Math.max(p.devPhysical||0, idx===0 ? 10 : 9);
    if(idx===0) p.cooldown=Math.min(p.cooldown||0,.12);
    p._bossTraitsApplied=true;
  });

  // Momotaro keeper reuses Oni graphic.
  if(gk){
    gk.oni=true;
    gk.momotaroGK=true;
    gk.speed=Math.max(gk.speed||0,210);
...
function rebuildPlayableBossTeamsNow(){
  const playable=
    gameMode==="free" ||
    gameMode==="daycup" ||
    gameMode==="deathmatch" ||
  ...
  // v164: never touch dedicated Oni/Momotaro battle identities here.
  if(!playable) return;
  ...
  if(blueKind) applyBossTraitsToSide("blue",selectedTeamId);
  if(redKind) applyBossTraitsToSide("red",opponentTeamId);
}

function forceRefreshPlayableBossTeams(){
  rebuildPlayableBossTeamsNow();
}

function applyPlayableBossTeamTraits(){
  // v164: this helper is ONLY for modes where unlocked boss teams are playable.
  // Dedicated Oni/Momotaro boss battles maintain their own rosters and must not
  // be cleared every physics frame.
  const playable=
    gameMode==="free" ||
    gameMode==="daycup" ||
    gameMode==="deathmatch" ||
...
function updatePeachCounterDefense(dt){
  if(gameMode!=="momotaro" || !momotaroState.peachShot || ball.owner) return;
  momotaroState.peachDefenseLock=Math.max(0,(momotaroState.peachDefenseLock||0)-dt);
  if(momotaroState.peachDefenseLock>0) return;

  const c=controlled();
  if(!c || dist(c,ball)>82 || ball.z>48) return;

  const trapPressed=input.trapPressBuffer>0;
  const shotPressed=input.actionPriorityTimer>0 && input.actionPriority==="shoot";
  if(!trapPressed && !shotPressed) return;

  momotaroState.peachDefenseLock=.30;
  momotaroState.peachShot=false;
  momotaroState.peachHitPost=false;
  ball.developedPierce=false;
  ball.developedPierceShooter=null;
  ...
  if(shotPressed){
    const gx=c.team==="blue"?COURT.x+COURT.w+18:COURT.x-18;
    const n=norm(gx-c.x,H/2-c.y);
    ball.owner=null; ball.lastTouch=c; ball.passFrom=c; ball.passTarget=null;
    ball.shot=true; ball.vx=n.x*1180; ball.vy=n.y*1180; ball.vz=0;
    c.kickAnim=Math.max(c.kickAnim||0,.18);
    showMessage("桃返し！",.65);
  }else{
    ball.owner=c; ball.lastTouch=c; ball.passTarget=null; ball.passFrom=null;
    ball.shot=false; ball.vx=ball.vy=ball.vz=0; ball.z=0; c.possessionTime=0;
    showMessage("桃止め！",.65);
  }
}

function updatePhysics(dt) {
  updatePeachCounterDefense(dt);
  if(ball.explosiveGKLock>0) ball.explosiveGKLock=Math.max(0,ball.explosiveGKLock-dt);

  // v164: Momotaro boss super armor.
  for(const mp of [...teams.blue,...teams.red]){
    if(hasMomotaroSuperArmor(mp)){
      if("stun" in mp) mp.stun=0;
      if("frozen" in mp) mp.frozen=0;
      if("freeze" in mp) mp.freeze=0;
      if("shock" in mp) mp.shock=0;
      if(mp.stagger>0 && mp.shoulder<=0 && mp.slide<=0) mp.stagger=0;
    }
  }

  applyPlayableBossTeamTraits();

  if(isOniHardPierce()){
    for(const gk of [...teams.blue,...teams.red]){
      if(gk.role==="gk" && dist(gk,ball)<30 && ball.z<80){
        blowThroughKeeper(gk);
      }
    }
  }
  ...
  // v164: in DEATHMATCH a loose ball must physically reach the feet.
...
function goal(who) {
  ...
  // v164: the team that conceded always takes the next kickoff.
...
function registerTimeStopDashTap(){
  ...
  // v164: independent hidden-skill counter.
...
function updateDuringTimeStop(dt){
  ...
  // v164: a shot during Oni Tag time stop is explicitly aimed at a nearby opponent.
  if(gameMode==="oni-tag" && !ball.owner && ball.lastTouch===c && ball.shot){
    const target=nearestOniTagTarget(c);
    if(target){
      const speed=Math.max(540,Math.hypot(ball.vx,ball.vy));
      const n=norm(target.x-ball.x,target.y-ball.y);
      ball.vx=n.x*speed;
      ball.vy=n.y*speed;
      ball.developedPierce=false;
      ball.developedPierceShooter=null;
  ...
  // Tag collision still resolves while everybody else is frozen.
  if(gameMode==="oni-tag") updateOniTag(dt);
...
function update(dt) {
  ...
  if(gamePhase==="match"){
    // v164: Oni Tag's actual match clock also stops during time stop.
    if(!(gameMode==="oni-tag" && timeStopTimer>0)){
  ...
  if(gameMode==="oni-tag"){
    for(const p of oniTagParticipants()){
      if(p.controlled) updateControlled(p,dt);
      else updateOniTagCPU(p,dt);
    }
  }else{
    for(const p of teams.blue) p.controlled?updateControlled(p,dt):(p.role==="gk"?updateGK(p,dt):updateAI(p,dt));
    for(const p of teams.red) p.controlled?updateControlled(p,dt):(p.role==="gk"?updateGK(p,dt):updateAI(p,dt));
  }
  ...
  if(gameMode==="momotaro" || momotaroState.peachShot) updateMomotaroBattle(dt);
  updatePhysics(dt);
  if(gameMode==="oni-tag") updateOniTag(dt);
}


function drawOniTagRoom(){
  if(gameMode!=="oni-tag") return false;
  ctx.save();
  ...
  ctx.strokeStyle="rgba(80,90,105,.12)";
  ctx.lineWidth=1;
  for(let x=COURT.x;x<=COURT.x+COURT.w;x+=80){
    ctx.beginPath();ctx.moveTo(x,COURT.y);ctx.lineTo(x,COURT.y+COURT.h);ctx.stroke();
  }
  for(let y=COURT.y;y<=COURT.y+COURT.h;y+=60){
    ctx.beginPath();ctx.moveTo(COURT.x,y);ctx.lineTo(COURT.x+COURT.w,y);ctx.stroke();
  }

  ctx.strokeStyle="#5f6877";
  ctx.lineWidth=10;
  ctx.strokeRect(COURT.x,COURT.y,COURT.w,COURT.h);

  ctx.strokeStyle="rgba(110,120,138,.45)";
  ctx.lineWidth=2;
  ctx.strokeRect(COURT.x+14,COURT.y+14,COURT.w-28,COURT.h-28);
...
function drawCourt() {
  if(drawOniTagRoom()) return;

  const greenField=gameMode==="deathmatch-dragon" || gameMode==="deathmatch-explosive" || gameMode==="oni-tag";
...
function drawPlayer(p) {
  ...
  // v164: dedicated-battle render fallback. Never lets the red boss roster appear purple.
  if(gameMode==="momotaro" && p.team==="red"){
    if(p.role==="gk" && !p.oni){
      const oldOni=p.oni, oldMGK=p.momotaroGK;
      p.oni=true; p.momotaroGK=true;
      if(p.slide>0){
        drawOniSlidingCharacter(p);
        p.oni=oldOni; p.momotaroGK=oldMGK;
        return;
      }
      // Standing GK continues through normal Oni overlay below.
      p._v164TempOni=oldOni;
      p._v164TempMGK=oldMGK;
    }else if(p.role!=="gk" && !p.momotaroRole){
      const field=teams.red.filter(q=>q.role!=="gk");
      const idx=field.indexOf(p);
      const oldRole=p.momotaroRole, oldMomo=p.momotaro;
      p.momotaro=true;
      p.momotaroRole=["momotaro","dog","monkey","pheasant"][idx] || "dog";
      if(p.slide>0) drawMomotaroSlidingCharacter(p);
      else drawMomotaroCharacter(p);
      p.momotaroRole=oldRole; p.momotaro=oldMomo;
      return;
    }
  }

  if(gameMode==="oni" && p.team==="red" && !p.oni){
    const oldOni=p.oni;
    p.oni=true;
    if(p.slide>0){
      drawOniSlidingCharacter(p);
  ...
  // v164: final render fallback based on the CURRENT selected team IDs.
  const renderKind=bossKindForTeamId(p.team==="blue"?selectedTeamId:opponentTeamId);

  if(renderKind==="momotaro" && p.role!=="gk" && !p.momotaroRole){
    const field=teams[p.team].filter(q=>q.role!=="gk");
    const ordered=[
      ...field.filter(q=>q.controlled),
      ...field.filter(q=>!q.controlled)
    ];
    const idx=ordered.indexOf(p);
    const oldRole=p.momotaroRole;
    const oldMomo=p.momotaro;
    p.momotaro=true;
    p.momotaroRole=["momotaro","dog","monkey","pheasant"][idx] || "dog";
    if(p.slide>0) drawMomotaroSlidingCharacter(p);
    else drawMomotaroCharacter(p);
    p.momotaroRole=oldRole;
    p.momotaro=oldMomo;
    return;
  }

  if(renderKind==="oni" && !p.oni){
    const oldOni=p.oni;
    p.oni=true;
    if(p.slide>0){
      drawOniSlidingCharacter(p);
    ...
    p._restoreOniAfterDraw=oldOni;
  }


  // v164 render-only fallback: never write identity flags here.
  const currentSideKind=bossKindForTeamId(p.team==="blue"?selectedTeamId:opponentTeamId);
  if(currentSideKind==="oni" && !p.oni){
    // draw as Oni without permanently changing the player object
    const oldOni=p.oni;
    p.oni=true;
    if(p.slide>0){ drawOniSlidingCharacter(p); p.oni=oldOni; return; }
    p.oni=oldOni;
  }

    // v164: dedicated sliding graphics for boss characters.
  if(p.slide>0){
  ...
  if(p.role==="gk" && p.gkFall>0){
    ...
    ctx.restore();
    if(p._restoreOniAfterDraw!==undefined){p.oni=p._restoreOniAfterDraw;delete p._restoreOniAfterDraw;}
    return;
  }
  ...
  if(p.stagger>0 && !p.oni) {
    ...
    ctx.arc(4,-30,1.7,0,Math.PI*2);
    ctx.fill();
    ctx.restore(); if(p._restoreOniAfterDraw!==undefined){p.oni=p._restoreOniAfterDraw;delete p._restoreOniAfterDraw;} return;
  }
  ...
  if(p.momotaroRole){
    ctx.save();
    // Momotaro: white/red headband. Companions: simple ear/crest silhouettes.
    if(p.momotaroRole==="momotaro"){
      ctx.fillStyle="#fff";
      ctx.fillRect(-11,-39,22,4);
      ctx.fillStyle="#d22";
      ctx.beginPath();ctx.arc(0,-37,3,0,Math.PI*2);ctx.fill();
    }else if(p.momotaroRole==="dog"){
      ctx.fillStyle="#8b5a2b";
      ctx.beginPath();ctx.moveTo(-9,-37);ctx.lineTo(-15,-48);ctx.lineTo(-3,-40);ctx.fill();
      ctx.beginPath();ctx.moveTo(9,-37);ctx.lineTo(15,-48);ctx.lineTo(3,-40);ctx.fill();
    }else if(p.momotaroRole==="monkey"){
      ctx.strokeStyle="#8b5a2b";ctx.lineWidth=4;
      ctx.beginPath();ctx.arc(0,12,18,-.3,1.7);ctx.stroke();
    }else if(p.momotaroRole==="pheasant"){
      ctx.fillStyle="#2f8f5b";
      ctx.beginPath();ctx.moveTo(0,-43);ctx.lineTo(5,-55);ctx.lineTo(9,-42);ctx.fill();
    }
    ctx.restore();
  }
  ...
  if(gameMode==="deathmatch" &&
    ...
    // v164: enemy bazooka is visibly held toward the left (its attacking direction).
  ...
  if(p._restoreOniAfterDraw!==undefined){
    p.oni=p._restoreOniAfterDraw;
    delete p._restoreOniAfterDraw;
  }

  if(p._v164TempOni!==undefined){
    p.oni=p._v164TempOni;
    delete p._v164TempOni;
  }
  if(p._v164TempMGK!==undefined){
    p.momotaroGK=p._v164TempMGK;
    delete p._v164TempMGK;
  }
}

function drawBall() {
  if(drawPeachBall()) return;
...
function draw() {
  ...
  const all=[...teams.blue,...teams.red]
    .filter(p=>(gamePhase!=="practice" || p.practiceActive) && (gameMode!=="oni-tag" || p.oniTagParticipant))
    .sort((a,b)=>a.y-b.y);
  for(const p of all) drawPlayer(p);
  if(gameMode==="deathmatch-explosive") drawExplosiveBall();
  else if(gameMode!=="oni-tag") drawBall();
  if(gameMode==="oni-tag") drawOniTagEffects();
...
bindMenuTap(secretInfoBtnEl,()=>{
  setMenuScreen(secretInfoScreenEl);
});
bindMenuTap(secretInfoBackBtnEl,()=>{
  setMenuScreen(modeScreenEl);
});
...
bindMenuTap(momotaroBattleBtnEl,()=>{
  selectedMatchSeconds=60;
  startMomotaroBattle();
});
...
bindMenuTap(oniTagBtnEl,()=>startOniTag());
...
window.addEventListener("DOMContentLoaded",()=>{
  ...
  if(v) v.textContent="v164";
  const b=document.getElementById("buildBadge");
  if(b) b.textContent="v164";
});
