...
function buildDevelopmentState(){
  // v109: selected team + 3 random opponents = 4-team single round robin.
...
function setMenuScreen(which){
  // v109: prevent the same touch from falling through into the newly shown screen.
...
function setupDeathmatchLines(){
  // v109: vertical hazard lines only, evenly spaced.
...
function bazookaAimDirection(p){
  // v109: forward is always the attacking direction, never toward own goal.
...
function attemptTrap(p, dt) {
  // v109: TRAP must not vacuum a loose ball from a distance.
  ...
  if(p.controlled) {
    ...
    if(input.trap || input.trapPressBuffer>0 || input.trapGraceTimer>0 ||
      ...
      // v109: never stop/snap a ball unless it is genuinely at the feet.
  ...
  if(!p.controlled) {
    ...
    if(success) {
      // v109: CPU also needs real contact before claiming/stopping the ball.
...
function aiWithBall(p,dt) {
  // v109: any AI field player may shoot when the chance is clearly good.
...
function updatePhysics(dt) {
  // v109: in DEATHMATCH a loose ball must physically reach the feet.
...
function drawPlayer(p) {
  ...
  if(gameMode==="deathmatch" &&
    ...
    // v109: enemy bazooka is visibly held toward the left (its attacking direction).
