...
function buildDevelopmentState(){
  // v150: selected team + 3 random opponents = 4-team single round robin.
...
function setMenuScreen(which){
  // v150: prevent the same touch from falling through into the newly shown screen.
...
function setupDeathmatchLines(){
  // v150: vertical hazard lines only, evenly spaced.
...
function bazookaAimDirection(p){
  // v150: forward is always the attacking direction, never toward own goal.
...
function startMatch(opponentId){
  ...
  // v150: roster has finished initializing; now force boss graphics/stats.

  // v150: all players now exist; rebuild boss identities from CURRENT selections.
...
function resetKickoff(team="blue") {
  ...
  // v150: preserve current boss-team identities after kickoff reset.

  // v150: identity assignment must happen last.
  if(gameMode==="momotaro"){
    applyDedicatedMomotaroRoster();
  }else if(gameMode==="oni"){
    applyDedicatedOniRoster();
...
function kickBall(p, dx,dy, speed, lift=0, shot=false, target=null) {
  ...
  // v150: Peach homing belongs only to Momotaro's own special shot.
...
function attemptTrap(p, dt) {
  ...
  // v150: TRAP must not vacuum a loose ball from a distance.
  ...
  if(p.controlled) {
    ...
    if(input.trap || input.trapPressBuffer>0 || input.trapGraceTimer>0 ||
      ...
      // v150: never stop/snap a ball unless it is genuinely at the feet.
  ...
  if(!p.controlled) {
    ...
    if(success) {
      // v150: CPU also needs real contact before claiming/stopping the ball.
...
function aiWithBall(p,dt) {
  // v150: any AI field player may shoot when the chance is clearly good.
...
function updateAI(p,dt) {
  // v150: controlled player must never receive CPU boss decisions.
...
function updateGK(p,dt) {
  // v150: 鬼蹴・剛 is absolute. No keeper save path may touch it.
  if(isOniHardPierce()){
  ...
  // v150: 鬼蹴・剛 and any developed pierce shot ignore ALL keeper save logic.
...
function rebuildPlayableBossTeamsNow(){
  // v150: clearing stale boss identities is universal.
...
function updatePhysics(dt) {
  ...
  // v150: Momotaro boss super armor.
  ...
  // v150: in DEATHMATCH a loose ball must physically reach the feet.
...
function registerTimeStopDashTap(){
  ...
  // v150: independent hidden-skill counter.
...
function drawPlayer(p) {
  ...
  // v150: dedicated-battle render fallback. Never lets the red boss roster appear purple.
  if(gameMode==="momotaro" && p.team==="red"){
    if(p.role==="gk" && !p.oni){
      ...
      p._v150TempOni=oldOni;
      p._v150TempMGK=oldMGK;
  ...
  // v150: final render fallback based on the CURRENT selected team IDs.
  ...
  // v150 render-only fallback: never write identity flags here.
  ...
  if(currentSideKind==="oni" && !p.oni){
    ...
    // v150: dedicated sliding graphics for boss characters.
  if(p.slide>0){
  ...
  if(gameMode==="deathmatch" &&
    ...
    // v150: enemy bazooka is visibly held toward the left (its attacking direction).
  ...
  if(p._v150TempOni!==undefined){
    p.oni=p._v150TempOni;
    delete p._v150TempOni;
  }
  if(p._v150TempMGK!==undefined){
    p.momotaroGK=p._v150TempMGK;
    delete p._v150TempMGK;
  }
}
...
window.addEventListener("DOMContentLoaded",()=>{
  ...
  if(v) v.textContent="v150";
  const b=document.getElementById("buildBadge");
  if(b) b.textContent="v150";
});
