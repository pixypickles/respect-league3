...
function buildDevelopmentState(){
  // v157: selected team + 3 random opponents = 4-team single round robin.
...
function setMenuScreen(which){
  // v157: prevent the same touch from falling through into the newly shown screen.
...
function setupDeathmatchLines(){
  // v157: vertical hazard lines only, evenly spaced.
...
function bazookaAimDirection(p){
  // v157: forward is always the attacking direction, never toward own goal.
...
function startMatch(opponentId){
  ...
  // v157: roster has finished initializing; now force boss graphics/stats.

  // v157: all players now exist; rebuild boss identities from CURRENT selections.
...
function resetKickoff(team="blue") {
  ...
  // v157: preserve current boss-team identities after kickoff reset.

  // v157: identity assignment must happen last.
  if(gameMode==="momotaro"){
    applyDedicatedMomotaroRoster();
  }else if(gameMode==="oni"){
    applyDedicatedOniRoster();
  ...
  // v157: Oni Tag room keeps nonparticipants hidden.
  if(gameMode==="oni-tag"){
    for(const p of [...teams.blue,...teams.red]){
...
function kickBall(p, dx,dy, speed, lift=0, shot=false, target=null) {
  ...
  // v157: Oni Tag SHOT always auto-aims at the nearest other player.
  ...
  // v157: Peach homing belongs only to Momotaro's own special shot.
  ...
  // v157: shooting during time stop releases the ball immediately.
  ...
  // v157: Oni Tag always uses tag-contact projectiles, never goal-piercing shots.
  if(gameMode==="oni-tag"){
    ball.developedPierce=false;
    ball.developedPierceShooter=null;
  }
...
function attemptTrap(p, dt) {
  ...
  // v157: TRAP must not vacuum a loose ball from a distance.
  ...
  if(p.controlled) {
    ...
    if(input.trap || input.trapPressBuffer>0 || input.trapGraceTimer>0 ||
      ...
      // v157: never stop/snap a ball unless it is genuinely at the feet.
  ...
  if(!p.controlled) {
    ...
    if(success) {
      // v157: CPU also needs real contact before claiming/stopping the ball.
...
function aiWithBall(p,dt) {
  // v157: any AI field player may shoot when the chance is clearly good.
...
function updateAI(p,dt) {
  // v157: controlled player must never receive CPU boss decisions.
  ...
  if(
    ...
    if(p.momotaroRole==="momotaro"){
      // v157: every CPU Momotaro possession becomes a peach shot.
...
function updateGK(p,dt) {
  // v157: 鬼蹴・剛 is absolute. No keeper save path may touch it.
  if(isOniHardPierce()){
  ...
  // v157: 鬼蹴・剛 and any developed pierce shot ignore ALL keeper save logic.
...
function rebuildPlayableBossTeamsNow(){
  // v157: clearing stale boss identities is universal.
...
function updatePhysics(dt) {
  ...
  // v157: Momotaro boss super armor.
  ...
  // v157: in DEATHMATCH a loose ball must physically reach the feet.
...
function registerTimeStopDashTap(){
  ...
  // v157: independent hidden-skill counter.
...
function updateDuringTimeStop(dt){
  ...
  // v157: a shot during Oni Tag time stop is explicitly aimed at a nearby opponent.
...
function update(dt) {
  ...
  if(gamePhase==="match"){
    // v157: Oni Tag's actual match clock also stops during time stop.
...
function drawPlayer(p) {
  ...
  // v157: dedicated-battle render fallback. Never lets the red boss roster appear purple.
  if(gameMode==="momotaro" && p.team==="red"){
    if(p.role==="gk" && !p.oni){
      ...
      p._v157TempOni=oldOni;
      p._v157TempMGK=oldMGK;
  ...
  // v157: final render fallback based on the CURRENT selected team IDs.
  ...
  // v157 render-only fallback: never write identity flags here.
  ...
  if(currentSideKind==="oni" && !p.oni){
    ...
    // v157: dedicated sliding graphics for boss characters.
  if(p.slide>0){
  ...
  if(gameMode==="deathmatch" &&
    ...
    // v157: enemy bazooka is visibly held toward the left (its attacking direction).
  ...
  if(p._v157TempOni!==undefined){
    p.oni=p._v157TempOni;
    delete p._v157TempOni;
  }
  if(p._v157TempMGK!==undefined){
    p.momotaroGK=p._v157TempMGK;
    delete p._v157TempMGK;
  }
}
...
window.addEventListener("DOMContentLoaded",()=>{
  ...
  if(v) v.textContent="v157";
  const b=document.getElementById("buildBadge");
  if(b) b.textContent="v157";
});
